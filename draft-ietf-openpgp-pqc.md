...
The secret key length depends on the secret key format specified in {{mlkem-seckey-id}}.
...
----------------------:| ----------- | ---------- | ---------- | ---------- | ---------
35                     | ML-KEM-768  | 1184       | 64 or 2400 | 1088       | 32
36                     | ML-KEM-1024 | 1568       | 64 or 3168 | 1568       | 32
...
The algorithm-specific secret key is this series of values:
 ...
 - A one-octet ML-KEM secret key format identifier defined in {{mlkem-seckey-id}}.

 - An octet string containing the ML-KEM secret key, whose length depends on the secret key format identifier in the previous field.

#### ML-KEM Secret Key Format Identifier {#mlkem-seckey-id}

{: title="ML-KEM Secret Key Format Identifier" #tab-mlkem-seckey-id}
ID | Name
-----:| ------------
0x00  | Seed Format
...
Upon parsing the seed format, or before using the secret key, for the expansion of the key, the function `ML-KEM.KeyGen_internal` [FIPS-203] has to be invoked with the parsed values of `d` and `z` as input.

The expanded key format is the value `dk` that is generated by `ML-KEM.KeyGen` [FIPS-203].

The length of the different formats is given in {{tab-mlkem-sec-size}}.

{: title="ML-KEM secret key length in octets depending on secret key format identifier" #tab-mlkem-sec-size}
ML-KEM       | Seed Format | Expanded Key Format
------------:| ------------ | ---------
ML-KEM-768   | 64           | 2400
ML-KEM-1024  | 64           | 3168

Implementations MUST be able to parse the seed format and MAY implement parsing of the expanded key format.
...
The secret key length depends on the secret key format specified in {{mldsa-seckey-id}}.
...
----------------------:| --------- | -----------| ---------- | ---------------
30                     | ML-DSA-65 | 1952       | 32 or 4032 | 3309
31                     | ML-DSA-87 | 2592       | 32 or 4896 | 4627
...
The algorithm-specific secret key is this series of values:
 ...
 - A one-octet ML-DSA secret key format identifier defined in {{mldsa-seckey-id}}.

 - An octet string containing the ML-DSA secret key, whose length depends on the secret key format identifier in the previous field.

#### ML-DSA Secret Key Format Identifier {#mldsa-seckey-id}

{: title="ML-DSA Secret Key Format Identifier" #tab-mldsa-seckey-id}
ID | Name
-----:| ------------
0x00  | Seed Format
...
Upon parsing the seed format, or before using the secret key, for the expansion of the key, the function `ML-DSA.KeyGen_internal` [FIPS-204] has to be invoked with the parsed value of `xi` as input.

The expanded key format is the value `sk` that is generated by `ML-DSA.KeyGen` [FIPS-204].

The length of the different formats is given in {{tab-mldsa-sec-size}}.

{: title="ML-DSA secret key length in octets depending on secret key format identifier" #tab-mldsa-sec-size}
ML-KEM       | Seed Format | Expanded Format
------------:| ------------ | ---------
ML-DSA-65    | 32           | 4032
ML-DSA-87    | 32           | 4896
...
- Add a secret key format identifier for ML-DSA and ML-KEM secret keys to allow both the seed and expanded key format.
