...
A secret key MAY be reused in the following case: A user holds already a secret key that is part of their certificate on a hardware token.
The user creates a new key for the same cryptographic algorithm and security level as the first key.
...
However, private key seeds, as they are specified here for ML-KEM, ML-DSA, and SLH-DSA, MUST NOT be reused across different cryptographic algorithms or different security levels of the same algorithm.
